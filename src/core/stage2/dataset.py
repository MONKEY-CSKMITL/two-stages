...
รองรับ 3 โจทย์ สลับได้ด้วยค่า task ตัวเดียว:
  ...
  task="3class"      ยุบเหลือ 3 ระดับ (0=ปกติ, 1=เสียหายปานกลาง [รวม grade 1,2],
                      2=เสียหายมาก [grade 3]) → ต่างจาก binary ตรงที่ severe ยัง
                      แยกออกมาต่างหาก ไม่ถูกรวมกับ mild/moderate เหมือน binary

สำคัญ: การแบ่งข้อมูล (split) ใช้ชุดเดิมทั้ง 3 โจทย์ — ยุบ label ตอนโหลดข้อมูลตรงนี้
ไม่ใช่ตอนแบ่ง เพราะถ้าแบ่งใหม่คนละแบบ จะเทียบผลข้ามโจทย์กันไม่แฟร์
...
VertebraDataset.__getitem__ คืนค่า 5 อย่าง (image, level_idx, metadata, label,
grade_4class) — metadata เป็นเวกเตอร์ศูนย์ถ้าไม่ได้เปิดใช้ (metadata_stats=None)
ส่วน grade_4class ติดมาด้วยเสมอเพื่อวิเคราะห์ผลตอนโหมดที่ยุบคลาส (binary, 3class)
ได้โดยตรง ไม่ต้องพึ่งลำดับแถวใน dataframe ที่เปราะบางถ้ามีการ shuffle
"""
...
TASKS = {
    ...
    "3class": {
        # ยุบ: grade 0 = คลาส 0 (ปกติ), grade 1+2 = คลาส 1 (เสียหายปานกลาง — รวม
        # mild กับ moderate เข้าด้วยกัน), grade 3 (+4 typo) = คลาส 2 (เสียหายมาก)
        # ต่างจาก binary ตรงที่ severe ยังแยกออกมาต่างหาก ไม่รวมกับ mild/moderate
