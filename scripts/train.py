...
input:  configs/xxx.yaml
            ไฟล์ตั้งค่าที่ระบุทุกอย่าง: ใช้ข้อมูลชุดไหน, backbone อะไร, โจทย์ 4 คลาสหรือ binary,
...
def per_grade_recall(targets_4class, preds, task):
    ...
    ทำไมต้องมี: ตอนใช้โจทย์ binary โมเดลอาจจับ "ปานกลาง+รุนแรง" ได้หมดแต่พลาด
    "เล็กน้อย" ทั้งหมด แล้วยังได้คะแนนรวมดูดีอยู่ — ฟังก์ชันนี้เปิดโปงกรณีแบบนั้น
    ให้เห็นว่าพลาดคลาสไหนไปจริงๆ
    ...
    for g in range(4):
        ...
        if task == "binary":
            # โจทย์ binary: ถือว่า "จับได้" เมื่อทายเป็นคลาส 1 (เสียหาย)
            # ยกเว้น grade 0 ที่ถือว่าถูกเมื่อทายเป็นคลาส 0 (ไม่เสียหาย)
            correct = (preds[mask] == 0) if g == 0 else (preds[mask] == 1)
        else:
            # โจทย์ 4 คลาส: ต้องทายตรงเป๊ะถึงจะถือว่าถูก
            correct = preds[mask] == g
